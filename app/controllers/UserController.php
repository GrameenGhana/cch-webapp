...
class UserController extends BaseController {
    ...
    public function __construct() {
        ...
        $this->isDistrictAdmin = User::isDistrictAdmin($ruserRole);
...
//0                    'District Admin' => 'District Admin'
            ...
            $zones = Zone::whereRaw("district_id", array($this->districtIds));
        } else {
            $facs = Facility::all();
        ...
        $this->zones = array();
        $this->zones['Unknown'][0] = 'Un Known';
        foreach ($zones as $k => $v) {
            $this->zones[$v->facility->name][$v->id] = $v->name;
        }

        foreach ($facs as $k => $v) {
            $d = District::find($v->district);
            if (in_array($d->name, array_keys($this->facilities))) {
                array_push($this->facilities[$d->name], $v);
                $this->pfacilities[$d->name][$v->id] = $v->name;
            } else {
                $this->facilities[$d->name] = array($v);
                $this->pfacilities[$d->name] = array($v->id => $v->name);
            }
        }
    ...
    public function create() {
        ...
        return View::make('users.create', array('groups' => $this->groups,
                    'roles' => $this->roles,
                    'devices' => $devices, 'zones' => $this->zones,
                    'pfacilities' => $this->pfacilities,
                    'facilities' => $this->facilities));
    }
...
//        $user = User::with('facilities')->find($id);
        ...
        if ($this->isDistrictAdmin) {
            ...
            $d = Device::whereRaw('status = ? or user_id=?', array('unallocated', $user->id))->get();
        $devices = array('0' => "Unknown");
        foreach ($d as $k => $v) {
            $devices[$v->id] = $v->tag . ' - ' . $v->type;
        }
        //$devices[0]='Unknown';
...
//            $checkloc = (Input::get('group')=='CCH' && preg_match('/.*?supervisor.*?/',strtolower(Input::get('role'))))
        ...
        if ($validator->fails()) {
            ...
            $statusChange = new UserStatusChange();
            $statusChange->user_id = $user->id;
...
//            'username' => 'required|min:3|unique:cch_users,username,' . $id,
            ...
            'last_name' => 'required|min:2',
            'group' => 'required|min:2',
            'role' => 'required|min:2', 'comment' => 'required'
        );
...
//            $user->username = Input::get('username');
            ...
            //Update the userstatus change table if there is any change in the status
             if ($user->status != $oldStatus) {
                $statusChange = new UserStatusChange();
                $statusChange->user_id = $user->id;
                $statusChange->old_status =$oldStatus;
                $statusChange->new_status =  $user->status;
                $statusChange->created_at =date('Y-m-d h:m:s');
                $statusChange->modified_by = Auth::user()->id;
                $statusChange->created_by  = Auth::user()->id;
