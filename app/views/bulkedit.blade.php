...
<section class="content-header">
    <h1> <i class="fa fa-hospital-o"></i> Zone Populations <small>Control panel</small> </h1>
    <ol class="breadcrumb">
        <li><a href="{{ URL::to('/') }}"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">Zone Populations</li>
    </ol>
</section>
@stop
...
<section class="content invoice">
    ...
    <div class="row">
        <div class="col-xs-12">
            <h2 class="page-header">
                <a class="btn btn-small btn-success" href="{{ URL::to('zonepopulations/create') }}"><i class="fa fa-plus-circle"></i> Add a Population</a>
            </h2>                            
        </div><!-- /.col -->
    </div>
    ...
    <div class="box">
        ...
        <div class="box-body table-responsive">
            {{ Form::open(array('url'=> 'zonepopulation/bulkedit','method'=>'post')) }}
            <input type="submit" value="Edit All"  class="btn btn-success btn-lg" />
            <input type="hidden" name="type" value="{{$type}}" />
            <input type="hidden" name="typeId" value="{{$typeId}}" />
            <input type="hidden" name="year" value="{{$year}}" />
            
           @if($type=="subdistrict_id")  
                    Sub District Populations &raquo; {{$district->name}}  &raquo; {{$subdistrict->name}} - <strong>{{$year}}</strong>
           
           @else 
                District Populations &raquo; {{$district->name}} - <strong>{{$year}}</strong>
           @endif
             <table id="bulkedittable" class="table table-bordered table-striped">
                <thead>
                    <tr>
                        ...
                        <th>Chn < 5 yrs<br />
                            {{ $districtpopulation->chn_less_than_5_yrs_ }}%</th>
                ...
                <script>
                 var columnsToLast=2;
                    ...
                    <tr>
                        <td>{{$subdistrict->name}}-{{$year}}</td>
                        <td></td>
                        <td></td>
                        <td>{{ $subdistrictpopulation->population }}</td>
                        <td>{{ $subdistrictpopulation->expected_pregnanciers }}</td>
                        <td>{{ $subdistrictpopulation->chn_6_11_mnths }}</td>
                        <td>{{ $subdistrictpopulation->chn_0_to_11_mnths }}</td>
                        <td>{{ $subdistrictpopulation->chn_12_23_mnths }}</td>
                        <td>{{ $subdistrictpopulation->chn_0_to_23_mnths }}</td>
                        <td>{{ $subdistrictpopulation->chn_24_to_59_mnths }}</td>
                        <td>{{ $subdistrictpopulation->chn_less_than_5_yrs_ }}</td>
                        <td>{{ $subdistrictpopulation->wifa_15_49_yrs }}</td>
                        <td>{{ $subdistrictpopulation->men_women_50_to_60_yrs }}</td>
                    </tr>
                    @elseif
                     <script>
                            var columnsToLast=1;
                    ...
                    <tr>
                        <td>{{$district->name}}-{{$year}}</td>
                        <td></td>
                        <td></td>
                        <td>{{ $districtpopulation->population }}</td>
                        <td>{{ number_format(($districtpopulation->expected_pregnanciers/100)* $districtpopulation->population ) }}</td>
                        ...
                        <td>{{ number_format(($districtpopulation->chn_less_than_5_yrs_/100)* $districtpopulation->population ) }}</td>
...
<script type="text/javascript">
    ...
    var tot = 0;
            ...
            $("#" + tableID + " tr:nth-last-child(-n+"+colsTolast+")").children("td:nth-child(" + columnIndex + ")").html(toFixed(tot, 2));
            return tot;
    }
